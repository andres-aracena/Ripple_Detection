...
# Bandpass Butterworth filter (100-200 Hz)
def bandpass_filter_gpu(data, lowcut=100, highcut=200, fs=30000, order=2):
    nyq = 0.5 * fs
    low = lowcut / nyq
    high = highcut / nyq
...
# Filter signal
filtered_data = bandpass_filter_gpu(signal_data, fs=fs)
...
refractory_period = int(refractory_time_sec * fs)  # 450 muestras
...
ylim_range = [(-600, 600), (-400, 400), (-200, 200), (-100, 100)]
...
for i, ax in enumerate(axes):
    ...
    ax.plot(signal_df['time'], signal_df['threshold'], color='blue', alpha=0.8, label='Señal Filtrada')
